...
- **Experimental Fine-Tuning:** Explored QLoRA (4-bit quantization) to stabilize symbolic reasoning and prevent model collapse.

## 🚀 Results
- **Prompt Engineering:** Successfully increased execution accuracy from **9.0% to 26.0%** without fine-tuning or increasing model size.
- **Fine-Tuning:** Validated a stable training pipeline for sub-billion parameter models using 4-bit NormalFloat (NF4) and Gradient Scaling.

## 📂 File Directory
- `Testing_Qwen_FinQA.ipynb`: **[NEW]** Consolidated research notebook containing evaluation loops, Layer 3 implementation, and the QLoRA fine-tuning experiment.
- `FinQA_Qwen.ipynb`: Initial research notebook containing baseline evaluation loops.
...
- `Future_Steps`: Strategic roadmap for future implementations with higher compute access.
- `Presentation.pptx`: Summary presentation covering project architecture and results.
